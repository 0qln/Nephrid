...
fn compute_attacks_corners_no_occupancy() {
    {
        let attacks = compute_attacks(squares::A1, Bitboard::empty());
        let expected = Bitboard { v: 0x1010101010101fe_u64 };
        assert_eq!(expected, attacks);
    }
    {
        let attacks = compute_attacks(squares::A8, Bitboard::empty());
        let expected = Bitboard { v: 0xfe01010101010101_u64 };
        assert_eq!(expected, attacks);
    }
    {
        let attacks = compute_attacks(squares::H1, Bitboard::empty());
        let expected = Bitboard { v: 0x808080808080807f_u64 };
        assert_eq!(expected, attacks);
    }
    {
        let attacks = compute_attacks(squares::H8, Bitboard::empty());
...
fn compute_attacks_center_with_occupancy() {
    ...
    let attacks = compute_attacks(squares::D4, occupancy);
...
fn compute_attacks_center_no_occupancy() {
    let attacks = compute_attacks(squares::D4, Bitboard::empty());
