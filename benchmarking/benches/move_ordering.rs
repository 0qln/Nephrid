...
fn bench_positions(c: &mut Criterion, group_name: &str, target: RtStage) {
    ...
    for (i, line) in csv_data.lines().skip(1).enumerate() {
        ...
        match pos.get_turn() {
            colors::WHITE => {
                bench_stage::<perspectives::White>(&mut group, &name, fen, &pos, &scorer, target)
            }
            colors::BLACK => {
                bench_stage::<perspectives::Black>(&mut group, &name, fen, &pos, &scorer, target)
            }
            _ => unreachable!(),
        }
    }
...
pub fn generate_captures_and_promos(c: &mut Criterion) {
    bench_positions(
        c,
        "generate_captures_and_promos",
